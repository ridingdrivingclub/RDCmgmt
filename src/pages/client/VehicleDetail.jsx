    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-8">
        <div>
          ...
          <div className="text-rdc-dark-gray mt-1">{vehicle.color}</div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <InfoCard label="Mileage" value={vehicle.mileage ? `${vehicle.mileage.toLocaleString()} mi` : null} />
        <InfoCard label="Location" value={vehicle.location} />
        <InfoCard
          label="Registration Due"
          value={vehicle.registration_expiry ? format(new Date(vehicle.registration_expiry), 'MMM d, yyyy') : null}
        />
        <InfoCard
          label="Insurance Renewal"
          value={vehicle.insurance_expiry ? format(new Date(vehicle.insurance_expiry), 'MMM d, yyyy') : null}
        />
        <InfoCard
          label="Last Service"
          value={vehicle.last_service_date ? format(new Date(vehicle.last_service_date), 'MMM d, yyyy') : null}
        />
      </div>
      ...
      {vehicle.special_instructions && (
        <div className="card p-6 mb-8">
          <h3 className="section-title mb-3">Special Instructions</h3>
          <p className="text-rdc-dark-gray">{vehicle.special_instructions}</p>
        </div>
      )}
            ...
            {serviceRecords.slice(0, 3).map((record) => (
              <div key={record.id} className="bg-rdc-cream rounded-lg p-4">
                <div className="flex items-start justify-between mb-2">
                  <span className="font-medium text-black">{record.service_type}</span>
                  <span className="text-sm text-rdc-taupe">
                    {format(new Date(record.service_date), 'MMM d, yyyy')}
                  </span>
                </div>
                <div className="text-sm text-rdc-dark-gray">{record.vendor_name}</div>
                {record.notes && (
            ...
            {documents.slice(0, 5).map((doc) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-black truncate">{doc.title}</div>
                  <div className="text-xs text-rdc-taupe capitalize">{doc.document_type}</div>
                </div>
              </a>
            ))}
          </div>
